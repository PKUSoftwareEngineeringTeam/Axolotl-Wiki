...
如果你仅是博客创作者，通常不需要直接修改模板文件。你可以通过修改配置文件和内容文件来定制博客的外观。你也许想要学习一些模板语法，以便在内容中使用自定义短代码或变量。那么下面的章节将对你有所帮助。

## 配置文件

Allay 模板的配置文件通常命名为 `allay.toml`。该文件定义了站点的基本信息和参数。例如：
...
[params]
# 这里的内容会给模板中使用 .site 变量访问
title = "Allay Blog"
```

你可以将 theme 指定为你想使用的主题名称，该主题应位于 `themes/` 目录下。
...
在此之前，最好了解一下[变量](variables)和[作用域](scope)的概念。接下来我们将学习allay基本语法。
...
这个简明的语法系统让你能轻松创建动态模板，同时保持代码的清晰和可维护性。更多详情请参见[命令和语法](./commands)，这个文档的最后还总结了allay所有的语法生成规则，建议仔细阅读。

## 实战

最后我们给出了一个[实战](practice)的例子，希望能给用户提供一个参考。
